...
function GetBoxFilterGradientResult = GetBoxFilterGradient(obj,authToken,dataset,field,time,filterwidth,spacing,points)
%GetBoxFilterGradient(obj,authToken,dataset,field,time,filterwidth,spacing,points)
%
...
%   
%     Output:
%       GetBoxFilterGradientResult = (ArrayOfVelocityGradient)
...
data.val.spacing.name = 'spacing';
data.val.spacing.val = spacing;
data.val.spacing.type = '{http://www.w3.org/2001/XMLSchema}float';
...
response = callSoapService( ...
    ...
    soapMessage);
GetBoxFilterGradientResult = parseSoapResponse(response);

% Fault message handling
if isfield(GetBoxFilterGradientResult, 'faultstring')
    error('faultcode: %s\nfaultstring: %s\n', ...
        GetBoxFilterGradientResult.faultcode, ...
        GetBoxFilterGradientResult.faultstring);
end
