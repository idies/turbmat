...
function GetDensityResult = GetDensity(obj,authToken,dataset,time,spatialInterpolation,temporalInterpolation,points)
%GetDensity(obj,authToken,dataset,time,spatialInterpolation,temporalInterpolation,points)
%
...
%   
%     Output:
%       GetDensityResult = (ArrayOfDensity)
...
response = callSoapService( ...
    ...
    soapMessage);
GetDensityResult = parseSoapResponse(response);

% Fault message handling
if isfield(GetDensityResult, 'faultstring')
    error('faultcode: %s\nfaultstring: %s\n', ...
        GetDensityResult.faultcode, ...
        GetDensityResult.faultstring);
end
