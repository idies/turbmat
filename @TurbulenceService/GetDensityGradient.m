...
function GetDensityGradientResult = GetDensityGradient(obj,authToken,dataset,time,spatialInterpolation,temporalInterpolation,points)
%GetDensityGradient(obj,authToken,dataset,time,spatialInterpolation,temporalInterpolation,points)
%
...
%   
%     Output:
%       GetDensityGradientResult = (ArrayOfVector3)
...
response = callSoapService( ...
    ...
    soapMessage);
GetDensityGradientResult = parseSoapResponse(response);

% Fault message handling
if isfield(GetDensityGradientResult, 'faultstring')
    error('faultcode: %s\nfaultstring: %s\n', ...
        GetDensityGradientResult.faultcode, ...
        GetDensityGradientResult.faultstring);
end